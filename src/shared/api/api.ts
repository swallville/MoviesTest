...
import {
  ...
  MovieVideosResponse,
} from "#/entities/movies/model/apiSchema";

/**
 * Fetch movies from API
 */
export const getMovies = async (
  params: ApiClientProps,
): Promise<MoviesResponse | { error: unknown }> => {
  if (isEmpty(params)) {
    return {
...
export const getMovieDetails = async (
  params: ApiClientProps,
): Promise<MovieDetailsResponse | { error: unknown }> => {
  if (isEmpty(params)) {
    return {
      error: "No parameters provided",
    };
...
export const getMovieVideos = async (
  params: ApiClientProps,
): Promise<MovieVideosResponse | { error: unknown }> => {
  if (isEmpty(params)) {
    return {
      error: "No parameters provided",
    };
