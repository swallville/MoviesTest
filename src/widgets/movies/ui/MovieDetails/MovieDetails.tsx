...
export const MovieDetails = ({ id }: MovieDetailsProps) => {
  ...
  const movieVideosRequested = moviesSelectors.useSelectedMovieVideosRequested();
  ...
  if (movie) {
    return (
        <>
            {/** movie title */}
            <Header goBackButton={false} defaultBackgroundColor="secondary" title={movie.title} />
                    ...
                    <section className="flex flex-row gap-4 max-h-42.5 md:max-w-2xl">
                        ...
                        <div className="flex flex-col justify-between">
                           <div className="flex flex-col">
                            ...
                            <button
                                type="button"
                                className="mt-4 inline-flex items-center rounded-xs bg-[#746A64] px-6 py-4 text-base leading-6 font-medium text-white focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#746A64] w-full h-14"
                                onClick={() => {
                                  if (isMovieFavorite) {
                    ...
                    <section className="flex flex-col gap-4 w-full">
                        ...
                        <div className="flex flex-col gap-2 overflow-x-auto w-full">
                            {movieVideos.map((video, index) => (
                                <VideoCard key={video.id} onClick={() => onVideoCardClick(video.key)} index={index} />
                            ))}
                        </div>
                    </section>
                </main>
            ...
            {selectedVideo && (
                <VideoPlayer selectedVideo={selectedVideo} onCloseVideo={onCloseVideo} />
            )}
        </>
    );
  }
