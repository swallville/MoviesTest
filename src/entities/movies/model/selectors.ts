...
export const selectors = {
  ...
  useMoviesCurrentPage: () =>
    useUnit($movies.map((store) => store.currentPage)),
  useMoviesRequested: () => useUnit(moviesRequested),
  useMovieDetailsRequested: () => useUnit(selectedMovieRequested),
  useMoviesRequestIsPending: () =>
    useUnit(moviesQuery.$status.map((status) => status === "pending")),
  useGetCurrentMovieDetails: () =>
    useUnit($movies.map((store) => store.currentMovieDetails)),
  ...
  useGetCurrentMovieVideos: () =>
    useUnit($movies.map((store) => store.currentMovieVideos)),
  useSetSelectedMovieVideos: () => useUnit(setSelectedMovieVideos),
  useIsMovieFavorite: (movieId: string) =>
    useUnit($movies.map((store) => store.favoriteMovies.includes(movieId))),
